...
function App() {
  ...
  useEffect(() => {
    if (isSupabaseConfigured) {
      ...
      }).catch(error => setToast(`No se pudo conectar con el servidor central: ${error.message}`)).finally(() => setSyncing(false))
      return
    }
...
function LoginScreen() {
  ...
  return <div className="login-page">
    <div className="login-aside">
      ...
      <div className="login-feature"><ShieldCheck size={19}/><div><strong>Datos protegidos</strong><small>Acceso seguro y permisos personalizados por usuario</small></div></div>
    </div>
...
function Directory({type,query,notify}:{type:'Clientes'|'Proveedores',query:string,notify:(s:string)=>void}){
  ...
  const save=(row:string[])=>{setRows(editing==='new'?[...rows,row]:rows.map(item=>item[1]===row[1]?row:item));if(isSupabaseConfigured)void saveCentralDirectory(type,row).catch(error=>notify(`No se pudo guardar en la base central: ${error.message}`));setEditing(null);notify(`${type.slice(0,-1)} guardado correctamente`)}
